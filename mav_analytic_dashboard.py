...
from mav_server_simple import MAVListener
...
class MAVAnalyticsDashboard(QMainWindow):
    ...
    def create_overview_tab(self) -> QWidget:
        ...
        self.metrics = {}
        for i, _widget in enumerate([layout.itemAt(j).widget() for j in range(layout.count())]):
            if isinstance(_widget, QFrame):
                for child in _widget.findChildren(QLabel):
                    if child.text() not in ["Total Messages", "Msg Rate (msg/s)", "Throughput (KB/s)", 
                                            "Connected Drones", "Loss Rate (%)", "Latency (ms)", "Jitter (ms)", "Message Types"]:
    ...
    def create_metric_card(self, title: str, value: str) -> QFrame:
        ...
        setattr(self, f"metric_{title.replace(' ', '_').replace('(', '').replace(')', '').lower()}", value_label)
    ...
    def create_per_drone_tab(self) -> QWidget:
        ...
        self.drone_table.setHorizontalHeaderLabels([
            "SysID", "Status", "Messages", "Bytes", 
            "Loss Rate (%)", "Latency (ms)", "Last Seen", "Address"
        ])
    ...
    def create_charts_tab(self) -> QWidget:
        ...
        # Throughput chart
        self.throughput_chart_view = self.create_line_chart("Throughput (KB/s)", "Time (s)")
    ...
    def update_dashboard(self):
        ...
        all_drones_report = self.mav_listener.server.get_all_drones_analytics(window_s=window_s)
    ...
    def update_charts_data(self, report: Dict[str, Any]):
        ...
        for i, value in enumerate(self.traffic_history):
            self.throughput_series.append(i, value * 30 / 1024)  # Approximate KB/s (30 bytes avg per msg)
        ...
        if self.traffic_history:
            max_throughput = max(self.traffic_history) * 30 / 1024 if self.traffic_history else 100
    ...
    def update_drone_table(self, all_drones_report: Dict[int, Dict]):
        ...
        for sysid in sorted(all_drones_report.keys()):
            ...
            addr_str = f"{drone.addr[0]}:{drone.addr[1]}" if drone and drone.addr else "N/A"
