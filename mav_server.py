...
class DetailTab(QWidget):
    ...
    def display_drone_details(self, sysid: int, status: DroneStatus):
        ...
        QTreeWidgetItem(
            system_item, ["Connection Event", status.connection_event or "N/A"]
        )
        QTreeWidgetItem(
            system_item, ["Armed Status", "Armed" if status.armed else "Disarmed"]
        )
        ...
        QTreeWidgetItem(
            altitude_item,
            ["Climb Rate", f"{status.vertical_speed:.2f}m/s {climb_direction}"],
        )
...
class MAVLinkDashboard(QMainWindow):
    ...
    def start_server(self):
        ...
        self.server = MAVLinkServerThread(
            host=self.server_host, port=self.server_port, timeout=10
        )

        # ===== CONNECT SIGNALS =====
        self.server.signal_emitter.drone_connected.connect(self.on_drone_connected)
        self.server.signal_emitter.drone_disconnected.connect(
            self.on_drone_disconnected
        )
...
def main():
    ...
    dashboard = MAVLinkDashboard(server_host="0.0.0.0", server_port=5566)
    dashboard.show()
